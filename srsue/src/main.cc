...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("usim.protection_scheme", bpo::value<string>(&args->stack.usim.protection_scheme)->default_value("null"), "SUPI protection scheme (SUCI): null or profile_a")
    ("usim.home_network_pubkey", bpo::value<string>(&args->stack.usim.home_network_pubkey)->default_value(""), "Home network public key as 32-byte hex (SUCI profile A)")
    ("usim.home_network_pubkey_id", bpo::value<int>(&args->stack.usim.home_network_pubkey_id)->default_value(0), "Home network public key identifier (0-255)")
    ("usim.routing_indicator", bpo::value<string>(&args->stack.usim.routing_indicator)->default_value("0000"), "Routing indicator (up to 4 decimal digits)")
